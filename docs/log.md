...
Flying the Last Exam through the real panel is where `npm run byhand` came from,
and every defect below came out of that first pass. Not one of them moved a win
rate, because the simulation was right each time and only the account of it was
wrong:
...
## Flying the cards by hand

`npm run hands` plays the other seven missions off their briefing cards, and six findings came
out of its first pass, not one of them a balance number. The first was mine rather than the
game's. Five of the six plans said everything they had to say in the first twenty seconds and
then stopped talking, so a battle that was going well decayed into two hulls against one with
the clock running, which reads as a stalemate in the simulation and is really a commander who
walked away. A finishing loop that keeps naming whatever of theirs is nearest and on the plot
was worth five wins in eighteen, and took Shoal from none in three to two.
